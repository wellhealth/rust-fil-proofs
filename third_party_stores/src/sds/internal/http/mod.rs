...
impl<R: Read> Multipart<R> {
    ...
    ///
    /// Returns `Ok(())` when all fields have been read, or the first error.
    pub fn foreach_entry_ret<F>(&mut self, mut foreach: F) -> io::Result<()>
    where
        F: FnMut(MultipartField<&mut Self>)->io::Result<()>,
    {
        loop {
            match self.read_entry() {
                Ok(Some(field)) => if let Err(e) = foreach(field) { return Err(e);},
                Ok(None) => return Ok(()),
                Err(err) => return Err(err),
            }
        }
