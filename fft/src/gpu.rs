...
mod test {
    ...
    fn cpu_distribute_powers(coeffs: &mut [Fr], g: Fr) {
        use rayon::iter::IndexedParallelIterator;

        let chunk_size = coeffs.len() / num_cpus::get();
        coeffs
    ...
    fn test_distribute_powers() {
        ...
        let coeffs = (0..1024usize * 1024 * 2)
            .into_iter()
            .map(|_| Fr::random(&mut rng))
            .collect::<Vec<_>>();

        let mut coeffs_cpu = coeffs.clone();
        cpu_distribute_powers(&mut coeffs_cpu, Fr::multiplicative_generator());

        let mut coeffs_gpu = coeffs;
        let src = std::fs::read_to_string("fft.c").expect("cannot read opencl source file");
        let fft = create_fft_program(&src, 0).expect("cannot create fft program");

        let mut gpu_buf = fft
            .create_buffer(coeffs_gpu.len())
            .expect("cannot create coeffs buffer");
        gpu_buf
            .write_from(0, &coeffs_gpu)
            .expect("cannot send coeffs to gpu");

        distribute_powers(&fft, &gpu_buf, Fr::multiplicative_generator())
            .expect("cannot use gpu to distribute powers");

        gpu_buf
            .read_into(0, &mut coeffs_gpu)
            .expect("cannot read coeffs from gpu");

        assert_eq!(coeffs_cpu, coeffs_gpu);
    }

    #[test]
    fn test_multiply_by_field() {
        let mut rng = rand::rngs::OsRng;
        let coeffs = (0..1024usize * 1024 * 2)
            .into_iter()
            .map(|_| Fr::random(&mut rng))
        ...
        let minv = Fr::random(&mut rng);
        let chunk_size = coeffs_cpu.len() / num_cpus::get();
        coeffs_cpu.par_chunks_mut(chunk_size).for_each(|chunk| {
            for it in chunk {
                it.mul_assign(&minv)
            }
        });
        ...
        multiply_by_field(&fft, &gpu_buf, minv).expect("cannot use gpu to distribute powers");
